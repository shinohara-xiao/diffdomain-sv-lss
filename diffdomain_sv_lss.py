...
from utils_lss import comp2domins_by_lsstest, loadtads, visualization, comp2domins_by_lsstest_withSVs
...
if(opts['dvsd']):
    if opts['one']:
        result = comp2domins_by_lsstest_withSVs(chrn=opts['<chr>'], start=int(opts['<start>']), end=int(opts['<end>']), reso=int(opts['--reso']), hicnorm=opts['--hicnorm'], fhic0=opts['<hic0>'], fhic1=opts['<hic1>'], min_nbin=int(opts['--min_nbin']), f=opts['--f'],fsvs_condition2 = opts['--fsvs'],SVs_file_condition2 = opts['--svsfile'])
    ...
    if opts['multiple']:
        ...
        def comp2domins_by_lsstest_parallel(i):
            tmp_res = comp2domins_by_lsstest_withSVs(chrn=tadb.iloc[i, 0], start=tadb.iloc[i, 1], 
        ...
        if __name__=='__main__':
            try:
                ...
                comp2domins_by_lsstest_parallel(0)
            finally:
                P = Pool(int(opts['--ncore']))
                note = []
                for i in range(tadb.shape[0]):
                    note.append(P.apply_async(comp2domins_by_lsstest_parallel, [i]))
                P.close()
                P.join()
                result = []
                for i in note:
