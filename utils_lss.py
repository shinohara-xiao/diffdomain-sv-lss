        ...
        try:
            ...
            for i in range(len(el)):
                ...
                if k == l:
                    ...
                    mat[k, l] = el[i].counts
                    mat[l, k] = el[i].counts
            #np.save(file = f'/lanec3_home/zhangx/diffDomain/data/MatrixFromHic/{fhic}_{region}.npy',arr = mat)
            return mat
        except IOError:
...
def lss_formula(Mat):
    ...
    try:
        ...
        # calculate the sum of lambdan
        # abs(z) >= abs(lambdan)
        #lambdan =  np.sum(w)/nd
        #lambdan = lambdan/np.sqrt(0.20)
        ...
        lambdan = (Gnf-ave[4])/np.sqrt(D2[4])
        ...
        lambdan = np.nan
        p = np.nan
    return nd, lambdan, p , np.amax(np.abs(w)) #, w

def comp2domins_by_lsstest(chrn, start, end, reso, hicnorm, fhic0, fhic1,min_nbin, f):
        mat0 = contact_matrix_from_hic(chrn, start, end, reso, fhic0, hicnorm)
        ...
        if not mat0 is None and not mat1 is None:
            ...
            if ind.sum() >= min_nbin:
                ...
                result = lss_formula(Diffmatnorm)

                domname = 'chr%s:%s-%s' % (chrn, start, end)
                # his = result[3]
                result = [chrn, start, end, domname, result[1], result[2], result[0] ,result[3]]
            else:
                domname = 'chr%s:%s-%s' % (chrn, start, end)
                print('The length of this TAD is too small at this resolution to be calculated !')
                result = [chrn, start, end, domname, np.nan, np.nan, np.nan]
                ...
                # F =((2*math.pi)**-1) * np.power(4-np.power(his,2),0.5)
                # plt.bar(his,F)
        else:
            domname = 'chr%s:%s-%s' % (chrn, start, end)
            result = [chrn, start, end, domname, np.nan, np.nan, np.nan]
...
def comp2domins_by_lsstest_withSVs(chrn, start, end, reso, hicnorm, fhic0, fhic1,min_nbin,f,min_ratio = 0.2,SVs_file_condition2 = 'nosvsfile',fsvs_condition2 = None):
    # SVs_file_condition2 and fsvs_condition2 are all under condition2 
    mat0 = contact_matrix_from_hic(chrn, start, end, reso, fhic0, hicnorm)
    mat1 = contact_matrix_from_hic(chrn, start, end, reso, fhic1, hicnorm)
    domname = '%s:%s-%s' % (chrn, start, end)
